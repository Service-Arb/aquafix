...
`<form method="post" action="/quote">` answered with a 303. It sits in the
...
| `app/` | Routes only, most of them a factory call from `@evinvest/kitstart/next`. `[locale]/` is the root layout (so `<html lang>` is right on the first byte); `[locale]/[location]/` the point's pages; `quote`, `og`, `health`, `sitemap`, `robots` the non-page routes. `brand.css` is generated by `npm run palette` and held to its source by a test. |
| `proxy.ts` | Which point and which language a request gets, before any route renders: kitstart's `createProxy`. |
| `src/shared` | Config (`site` — the composition root every brand fact is read from —, the `i18n` registry, the server env), money and rating formatting, the lock-up and the band head. |
...
that is not a page or one of kitstart's `NON_PAGE_ROUTES` goes the same way,
with or without an extension (the site lists no `publicFiles`): let through,
...
natively; the site reads it at build (kitstart's `withLanding`, inlined
...
**Live data is an overlay, not a dependency.** The place source merges
`LOCATIONS_API_URL`'s answer over the baked point, with the TTL on the fetch
itself. The source withdrawing a point — a 410, or a 404 in its own JSON — is
`notFound()`; a bare 404 (an ingress with no route yet), a 5xx or an
unreachable source serves the baked point (a cold render that threw would get Next's bare-text 500, with no
...
`cloudflare`, the tunnel) have no default: without either, every request,
`/health` included, is a 500
