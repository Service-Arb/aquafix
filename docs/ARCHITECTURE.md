...
| `src/entities/lead` | The notifier, plus the slice API over the lead: its type and schema (`LEAD` in `shared/config/lead.ts`, the core in `shared/landing/core/lead.ts`) and the `LeadStore` port with its adapters (`shared/landing/server/lead-store*.ts`: SQLite today, chosen by the scheme of `LEADS_DB_URL`; `LEADS_DB_PATH` still names the SQLite file). Every adapter passes `tests/support/lead-store-contract.ts`. |
