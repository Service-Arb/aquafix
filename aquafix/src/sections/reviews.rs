...
use crate::{
	blocks::{CARD, Eyebrow, Head, Section, TILE, Tone},
	content::Lang,
};
		...
		Section { tone: Tone::Subtle, id: "reviews",
			div { class: "flex flex-col gap-4 md:gap-6",
				...
				div {
					...
					class: "flex flex-col gap-4 {CARD} border-[1.5px] border-dashed border-accent px-5 pb-[18px] pt-4 md:px-6",
					p { class: "text-[10.5px] font-medium tracking-[0.12em] text-accent",
						"{t.reviews_embed_note}"
					}
					div { class: "flex flex-col gap-5 md:flex-row",
						for review in t.reviews {
							// The mobile frame shows only the four-star card.
							div { class: "flex flex-1 flex-col gap-3.5 {TILE} border border-rule bg-surface px-6 pb-6 pt-[22px] {mobile_visibility(review.stars)}",
								p { class: "text-[17px] text-action",
									{"★".repeat(review.stars as usize)}
									{"☆".repeat(5 - review.stars as usize)}
								}
								p { class: "text-[15px] leading-[1.65] text-ink",
									"{review.body}"
								}
