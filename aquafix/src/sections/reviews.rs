...
use dioxus::prelude::*;
use ev_lib::uikit::{Card, Display, Eyebrow, Section, Surface};
		...
		Section { surface: Surface::Card, id: "reviews",
			div { class: "flex flex-col gap-4 md:gap-6",
				...
				div {
					...
					class: "flex flex-col gap-4 rounded-[var(--corner-card)] border-[1.5px] border-dashed border-primary px-5 pb-[18px] pt-4 md:px-6",
					...
					div { class: "flex flex-col gap-5 md:flex-row",
						for review in t.reviews {
							...
							Card { class: "flex-1 gap-3.5 rounded-[var(--corner-tile)] bg-background px-6 pb-6 pt-[22px] {mobile_visibility(review.stars)}",
