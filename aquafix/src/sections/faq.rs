...
pub fn Faq(lang: Lang) -> Element {
	...
	rsx! {
		Section { surface: Surface::Card, tight: true, id: "faq",
			div { class: "flex flex-col gap-6 md:gap-11",
				div { class: "flex flex-col gap-2 md:gap-3.5",
					Eyebrow { "{eyebrow}" }
					Display { "{title}" }
				}
				div { class: "overflow-hidden rounded-[var(--corner-card)] border border-border bg-background",
					for (i , faq) in t.faqs.iter().enumerate() {
						details {
							...
							summary { class: "flex items-start gap-4 px-5 py-5 md:px-[30px] md:pb-[26px] md:pt-6",
								...
								span { class: "text-[18px] text-primary", "+" }
							}
