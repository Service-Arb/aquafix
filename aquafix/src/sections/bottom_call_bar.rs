...
pub fn BottomCallBar(lang: Lang) -> Element {
	let t = lang.text();
	rsx! {
		div { id: "callbar", class: "fixed inset-x-0 bottom-0 z-30 flex gap-2.5 border-t border-rule bg-surface px-3 py-2.5 shadow-[0_-6px_20px_0_rgba(0,13,31,0.18)] md:hidden",
			a {
				href: SITE.tel_href(),
