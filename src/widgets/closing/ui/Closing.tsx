 ...
 */
export function Closing({ copy, point, renderedAt }: { copy: Copy; point: PlaceView; renderedAt: number }) {
  const { t, f } = copy;
    ...
    <Section polarity="dark" id="quote-band">
      <div className="flex flex-col gap-8 md:flex-row md:items-start md:gap-14">
        <div className="flex flex-1 flex-col gap-4 md:gap-5 md:pt-2">
          ...
          <a href={telHref(phone)} className="font-display text-[22px] font-bold text-primary-ink md:text-[26px]">
            {phone}
          </a>
        </div>
