...
describe("the event model", () => {
  it("refuses a customer's phone or address as a property in development", () => {
    const sink = analyticsSink({ key: null, host: "https://eu.i.posthog.com" }, "royat");
  ...
  it("sends nothing without a key", () => {
    ...
    analyticsSink({ key: null, host: "https://eu.i.posthog.com" }, "royat").capture(EVENTS.intent, { channel: "phone" });
  ...
  it("beacons the intent with the brand and the point mixed in, and no cookie", () => {
    ...
    vi.stubGlobal("navigator", { sendBeacon: beacon });
    analyticsSink({ key: "phc_test", host: "https://eu.i.posthog.com" }, "royat").capture(EVENTS.intent, { channel: "whatsapp" });
